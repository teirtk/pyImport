...
def get_date(df):
    ...
    for s in [s for s in df.columns if isinstance(s, str)]:
        result = get_date_str(s)
        if result is not None:
            return result
    for row in df.itertuples():
        for s in [s for (_, s) in enumerate(row) if isinstance(s, str)]:
            result = get_date_str(s)
            if result is not None:
                return result
    return None
